...
export const DepotForm: React.FC<DepotFormProps> = ({
  ...
  // Sync selectedCompteId or set default when comptes load
  useEffect(() => {
    if (selectedCompteId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || "Une erreur est survenue lors du dépôt.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="form-control">
        <label className="label">
          <span className="label-text font-medium text-base-content/75">Compte destinataire</span>
        </label>
        <select
      ...
      <div className="form-control">
        <label className="label">
          <span className="label-text font-medium text-base-content/75">Montant du dépôt (€)</span>
        </label>
        <input
          type="number"
          step="0.01"
